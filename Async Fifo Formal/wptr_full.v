module wptr_full #(parameter ADDR_WIDTH = 4)(
    ...
    output reg [ADDR_WIDTH :0] wptr,     // Write pointer
    input [ADDR_WIDTH :0] wq2_rptr,      // Read pointer (gray) - synchronised to write clock domain
    input winc, wclk, wrst_n            // Write increment, write-clock, and reset
    );

    reg [ADDR_WIDTH:0] wbin;                     // Binary write pointer
    wire [ADDR_WIDTH:0] wgray_next, wbin_next;   // Next write pointer in gray and binary code
    ...
    assign waddr = wbin[ADDR_WIDTH-1:0];             // Write address calculation from the write pointer
    ...
    // assign wfull_val=((wg_next[ADDR_WIDTH] !=wq2_rptr[ADDR_WIDTH] ) &&
    // (wg_next[ADDR_WIDTH-1] !=wq2_rptr[ADDR_WIDTH-1]) &&
    // (wg_next[ADDR_WIDTH-2:0]==wq2_rptr[ADDR_WIDTH-2:0]));
    //------------------------------------------------------------------
