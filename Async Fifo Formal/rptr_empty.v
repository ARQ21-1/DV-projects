...
module rptr_empty #(parameter ADDR_WIDTH = 4)(
    ...
    output reg [ADDR_WIDTH :0] rptr,     // Read pointer
    input [ADDR_WIDTH :0] rq2_wptr,      // Write pointer (gray) - synchronised to read clock domain
    input rinc, rclk, rrst_n            // Read increment, read-clock, and reset
    );

    reg [ADDR_WIDTH:0] rbin;                     // Binary read pointer
    wire [ADDR_WIDTH:0] rgray_next, rbin_next;   // Next read pointer in gray and binary code
    ...
    assign raddr = rbin[ADDR_WIDTH-1:0];                 // Read address calculation from the read pointer
